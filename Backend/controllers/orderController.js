...
exports.placeOrder = async (req, res, next) => {
  try {
    ...
    if (!items || items.length === 0) {
      return next(
        new errorHandler(400, "Order must contain at least one item.")
      );
    }
    ...
    for (const orderItem of items) {
      ...
      if (!menuItem) {
        return next(
          new errorHandler(404, `Item not found: ${orderItem.menu_item}`)
        );
      }
      ...
      if (!menuItem.isFreshlyMade) {
        if (menuItem.stock === null || menuItem.stock < orderItem.quantity) {
          return next(
            new errorHandler(400, `Not enough stock for ${menuItem.name}`)
          );
        }
...
exports.getAllOrders = async (req, res, next) => {
  try {
    ...
    if (paymentStatus) {
      ...
      const payments = await require("../models/payments")
        .find(paymentQuery)
        .select("order");
    ...
    const [orders, total] = await Promise.all([
      Order.find(query)
        .populate({
          path: "items.menu_item", // ✅ ADD THIS
          select: "name price description category image", // ✅ ADD THIS
        })
        .populate("payment")
        .sort(sort)
        .skip(skip)
...
exports.updateOrderStatus = async (req, res, next) => {
  try {
    ...
    if (order.status === "completed") {
      return next(
        new errorHandler(400, "Cannot modify status of a completed order")
      );
    }
...
exports.addToOrder = async (req, res, next) => {
  try {
    ...
    if (order.status === "completed" || order.status === "cancelled") {
      return next(
        new errorHandler(400, "Cannot modify a completed or cancelled order.")
      );
    }
    ...
    for (const orderItem of items) {
      ...
      const existingItemIndex = updatedItems.findIndex(
        (item) => item.menu_item.toString() === orderItem.menu_item.toString()
      );
